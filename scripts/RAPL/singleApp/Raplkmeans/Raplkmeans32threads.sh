...
for i in {0..40}
     ...
     while [[ $HR = '' ]]||[[ $power = '' ]]||[[ $joules = '' ]]||[[ $c -le 0 ]]
      ...
      $POWER_MON start
     sudo -E bash -c 'export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/home/huazhe/intel64;export HEARTBEAT_ENABLED_DIR=heartenabled/; /var/tmp/NU-MineBench-3.0/src/kmeans_test/example -i /var/tmp/NU-MineBench-3.0/datasets/kmeans/edge -b -o -f -p 32 -n 2 -m 3'
  ...
  #    echo $i $j $k $HR $power2 $joules2 $power $joules >> shmoo2.results
      ...
      echo $m $HR $power2 $power $joules >> Raplkmeans32threads.results
