...
class PowerControl:
    ...
    def PerfBSearch(self,lowbound, highbound):
        head = lowbound
        tail = highbound
        self.PerfDictionary[(head,16,2)],self.PwrDictionary[(head,16,2)] = self.GetFeedback((head,16,2))
        self.PerfDictionary[(tail,16,2)],self.PwrDictionary[(tail,16,2)] = self.GetFeedback((tail,16,2))
    ...
    def AdjustConfig(self, CoreNumber,freq,MemoCtrl):
        ...
        if self.CurCore != CoreNumber:
            ...
            #    os.system("for i in $(pgrep "+self.AppName+" | xargs ps -mo pid,tid,fname,user,psr -p | awk 'NR > 2  {print $2}');do sudo taskset -pc 0-"+str(CoreNumber-1)+" $i > /dev/null & done")
                ...
                result1 = subprocess.check_output("for i in $(pgrep "+self.AppNameShort+" | xargs ps -mo pid,tid,fname,user,psr -p | awk 'NR > 1  {print $2}');do sudo taskset -pc 0-7,16-"+str(CoreNumber-17)+" $i & done",shell=True)
        self.GetPowerDistAndSet(CoreNumber)
        self.CurCore = CoreNumber
        self.CurFreq = freq
        EndTime = time.time()
...
#j =0;for i in $(pgrep iterate | xargs pstree -p|grep -o "([[:digit:]]*)" |grep -o "[[:digit:]]*");do sudo taskset -pc 0-15 $i; j=`expr $j + 1`; done
