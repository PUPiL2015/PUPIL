...
for benchmark_name in benchmarkName:
    ...
    for configuration in config:
        core_number = int(configuration[1]) + int(configuration[2])
        dvfs_level = int(configuration[3])
        MC_number = int(configuration[4])
        line=''
        for line1 in linesr:
            liner =line1.split()

            if int(liner[0]) == core_number and int(liner[1]) == dvfs_level and int(liner[2]) == MC_number:
                line = line1
                break
        filew.write(line)
    filew.close()
    filer.close()
    #line_num = core_number* 32 +dvfs_level *2 + MC_number
